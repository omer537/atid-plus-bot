...
if user_input := st.chat_input("הקלד כאן..."):
    ...
    if st.session_state.stage == "setup_name":
        ...
        response_text = "אחלה. על איזה נושא אנחנו בונים את השיעור היום?"
        ...
        if is_approved:
            ...
            with st.spinner("סבבה, בוא נעשה בדיקה זריזה..."):
                 q1 = generate_response(f"תתחיל את הסימולציה הפדגוגית. הצג שאלה 1 מתוך {MAX_QUESTIONS}.", context=st.session_state.lesson_plan_text, use_fast_model=True)
                 response_text = f"**בדיקת מוכנות פדגוגית ({MAX_QUESTIONS} שאלות)**\n\n" + q1
        else:
            with st.spinner("אין בעיה, משפרים..."):
                ai_response = generate_response(f"הערת המשתמש: {user_input}. ערוך מחדש את המערך.", context=st.session_state.lesson_plan_text, use_fast_model=False)
                st.session_state.lesson_plan_text = ai_response
                response_text = ai_response + "\n\n**איך הגרסה הזו?**"
        ...
        if st.session_state.question_count < MAX_QUESTIONS:
            ...
            with st.spinner("בודק..."):
                response_text = generate_response(f"תשובה: '{user_input}'. משוב קצר ושאלה פדגוגית {q_num}.", context=st.session_state.lesson_plan_text, use_fast_model=True)
        else:
            with st.spinner("מסכם..."):
                feedback = generate_response(f"תשובה אחרונה: '{user_input}'. סיכום קצר.", context=st.session_state.lesson_plan_text, use_fast_model=True)
